...
- factor_count: `6`
- cost_bps: `15.0`
- extension_date_count: `51`
- extension_date_range: `2026-01-05` to `2026-03-27`
...
- No factor should advance as a broad all-candidate signal from this run.
- `order_unique_trade_participation_gap` is the closest broad candidate: base stress, time slices, and three primary layers pass, but both Southbound named buckets fail the stricter split gate.
- `avg_trade_notional_bias` only survives as an inverse candidate in `southbound_unknown`; it should be split away from eligible/core flow before any promotion.
- The other four candidates are layer-specific and should be rewritten as layer-explicit specs, not rebranded as broad factors.
