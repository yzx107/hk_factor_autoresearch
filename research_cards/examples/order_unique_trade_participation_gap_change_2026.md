+++
card_id = "rc_20260330_order_unique_trade_participation_gap_change_2026"
name = "订单-成交参与广度差变化（Order Unique Trade Participation Gap Change 2026）"
owner = "codex"
status = "draft"
...
hypothesis = '当 unique order breadth 相对 trade_count 异常抬升时，可能代理更强的未成交压力或更慢的成交吸收。 这里先看它的一日变化，而不是静态水平。'
mechanism = '比较唯一订单参与广度与成交笔广度，捕捉“挂单参与”和“实际成交实现”之间的落差。 变化版更关注压力切换，而不是常态水平。'
...
baseline_refs = ['order_unique_trade_participation_gap', 'order_trade_event_ratio', 'structural_activity_proxy']
...
当 unique order breadth 相对 trade_count 异常抬升时，可能代理更强的未成交压力或更慢的成交吸收。 这里先看它的一日变化，而不是静态水平。

## Mechanism

比较唯一订单参与广度与成交笔广度，捕捉“挂单参与”和“实际成交实现”之间的落差。 变化版更关注压力切换，而不是常态水平。
...
它比较的是订单参与广度和成交实现广度，不是单看 activity，也不是单看 order_event_count。 变化版优先检验它是否比对应 level 版更有增量。
