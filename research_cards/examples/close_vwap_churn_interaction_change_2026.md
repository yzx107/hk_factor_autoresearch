+++
card_id = "rc_20260330_close_vwap_churn_interaction_change_2026"
name = "收盘-VWAP 与 churn 交互变化（Close Vwap Churn Interaction Change 2026）"
owner = "codex"
status = "draft"
...
hypothesis = '如果 close-vwap gap 和 churn 同时抬升，可能比单独的尾盘压力或 churn 更像短周期未消化状态。 这里先看它的一日变化，而不是静态水平。'
mechanism = '把尾盘价格偏离和订单生命周期 churn 放在一起，捕捉“价格压力 × 订单摩擦”的联合状态。 变化版更关注压力切换，而不是常态水平。'
...
baseline_refs = ['close_vwap_churn_interaction', 'close_vwap_gap_intensity', 'order_lifecycle_churn']
...
如果 close-vwap gap 和 churn 同时抬升，可能比单独的尾盘压力或 churn 更像短周期未消化状态。 这里先看它的一日变化，而不是静态水平。

## Mechanism

把尾盘价格偏离和订单生命周期 churn 放在一起，捕捉“价格压力 × 订单摩擦”的联合状态。 变化版更关注压力切换，而不是常态水平。
...
它把 close-vwap pressure 和 order_lifecycle pressure 做交互，不是任一单家族的 level 重复。 变化版优先检验它是否比对应 level 版更有增量。
