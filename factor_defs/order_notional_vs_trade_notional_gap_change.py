...
FACTOR_ID = "order_notional_vs_trade_notional_gap_change_v1"
FACTOR_FAMILY = "order_trade_interaction_pressure"
MECHANISM = "Measure one-day acceleration in 订单-成交平均名义额差."
...
EXPECTED_REGIME = "change version of: works better when displayed order size rises faster than realized matched trade size"
...
OUTPUT_COLUMN = "order_notional_vs_trade_notional_gap_change_score"
LOOKBACK_STEPS = 1


def _daily_base(daily: pl.LazyFrame) -> pl.LazyFrame:
    return daily.with_columns(
        (((pl.col("total_order_notional").log1p() - pl.col("unique_order_ids").log1p()) - (pl.col("turnover").log1p() - pl.col("trade_count").log1p()))).alias("order_notional_vs_trade_notional_gap_level")
    )
...
def compute_signal(
    ...
    *,
    ...
    return build_change_signal(
        _daily_base(daily),
        base_score_column="order_notional_vs_trade_notional_gap_level",
