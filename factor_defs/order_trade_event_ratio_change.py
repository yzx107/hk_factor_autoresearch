"""One-day change version of order-to-trade event ratio."""
...
FACTOR_ID = "order_trade_event_ratio_change_v1"
FACTOR_FAMILY = "order_trade_interaction_pressure"
MECHANISM = "Measure acceleration in order-event intensity relative to matched trade intensity."
...
EXPECTED_REGIME = "works better when quoting pressure changes abruptly relative to matched execution"
...
OUTPUT_COLUMN = "order_trade_event_ratio_change_score"
LOOKBACK_STEPS = 1
...
def _daily_base(daily: pl.LazyFrame) -> pl.LazyFrame:
    return daily.with_columns(
        (
            pl.col("order_event_count").log1p() - pl.col("trade_count").log1p()
        ).alias("order_trade_event_ratio_level")
    )
...
def compute_signal(
    ...
    *,
    ...
    return build_change_signal(
        _daily_base(daily),
        base_score_column="order_trade_event_ratio_level",
